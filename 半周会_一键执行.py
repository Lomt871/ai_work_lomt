...
def run_overseas_report(page, download_dir: Path, module_haiwai, module_yixu):
    print("\n" + "=" * 60)
    print("开始下载：海外团队达成")
    print("=" * 60)
    ...
    # 海外脚本没有独立 export_excel，复用一续脚本中的通用导出实现
    module_yixu.export_excel(page, download_dir, module_haiwai.REPORT_FILENAME)


def main():
    mod_haiwai = load_module("海外团队达成_周报.py", "halfweek_haiwai")
    mod_jinsanyue = load_module("近三月结课续费率_周报.py", "halfweek_jinsanyue")
    mod_yixu = load_module("一续升舱续费率_周报.py", "halfweek_yixu")
    mod_tonghe = load_module("统合早鸟续费率_周报.py", "halfweek_tonghe")
    mod_summary = load_module("半周会分析结果_生成.py", "halfweek_summary")

    download_dir = mod_yixu.resolve_download_dir()
    os.environ["WEEKLY_REPORT_DOWNLOAD_DIR"] = str(download_dir)
    os.environ["WEEKLY_REPORT_MODE"] = "halfweekly"
    os.environ["WEEKLY_REPORT_INCLUDE_POOL_GMV"] = "0"
    os.environ["WEEKLY_REPORT_USE_MONTH_TARGET_ONLY"] = "1"

    print("\n" + "=" * 60)
    print("半周会一键执行开始")
    print("前4个脚本将复用同一次 SmartBI 登录")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print("\n1/4 海外团队达成")
            run_overseas_report(page, download_dir, mod_haiwai, mod_yixu)
    ...
    print("\n=== 前4个下载脚本执行结束，开始执行半周会分析结果生成 ===")
    try:
        mod_summary.main()
    except Exception as exc:
        print(f"半周会分析结果_生成执行失败: {exc}")
        traceback.print_exc()
        raise
